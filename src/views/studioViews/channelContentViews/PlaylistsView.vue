...
for (let i = 1; i <= 3; i++) {
    playlists.push({ id: i, title: `${i} Playlist title`, description: `${i} Playlist description` })
}
        ...
        <div v-for="playlist in playlists" :key="playlist.id" class="table-itself border-b">
            ...
            <div class="video-detail relative flex flex-col font-normal text-[13px] mt-2" style="padding-left: 12px;">
             ...
             justify-center">
                    ...
                    <button @click="toggleVideoOptions(playlist.id)" class="w-[39.2px] h-[39.2px] rounded-full hover:bg-[#eaeaea] flex items-center
             justify-center">
                        <img class="w-[17px] h-[17px] center" :src="kebabMenuIcon" alt="">
                    </button>
                    <div v-if="videoOptionStates[playlist.id]?.optionDiv" class="absolute left-40 top-0 video-edit-options w-[226px] h-auto rounded-xl bg-white flex flex-col
                 justify-start items-center py-4">
                        <div @click="toggleVideoEdit(playlist.id, playlist.title, playlist.description)" class="cursor-pointer w-full h-[32px] hover:bg-[#f9f9f9] flex flex-row justify-start items-center
                 ...
                 text-[15px] font-normal font-roboto">
                    ...
                    <div v-if="videoOptionStates[playlist.id]?.editDiv" class="edit-title-description bg-white z-40 w-[488px] h-[368px] rounded-lg
                 flex flex-col justify-start pt-2 items-center gap-y-4 absolute left-2 top-6">
                     ...
                     gap-x-2 pr-2 pb-2">
                            ...
                            <button @click="toggleVideoEdit(playlist.id)" class="w-[74.9px] h-[36px] rounded-3xl text-[14px] font-medium
